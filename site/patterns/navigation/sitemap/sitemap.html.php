<ul class="list-unstyled">
  <?php foreach ($pages->visible()->filterBy('slug', '!=', 'module') as $p): ?>
    <li>
      <a href="<?php echo $site->find($p)->url() ?>"><?php echo $site->find($p)->title() ?></a>
      <?php if($p->hasChildren()): ?>
        <ul class="list">
          <?php foreach($p->children()->visible()->filterBy('slug', '!=', 'module') as $c): ?>
            <li>
              <a<?php e($c->isOpen(), ' class="active"') ?> href="<?php echo $c->url() ?>"><?php echo $c->title()->html() ?></a>
              <?php if($c->hasChildren()): ?>
                <ul class="list">
                  <?php foreach($c->children()->visible()->filterBy('slug', '!=', 'module') as $g): ?>
