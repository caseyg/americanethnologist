...
<div class="row">
  ...
  <div class="col-sm-8 pull-sm-3">
    <?php foreach($site->index()->visible()->filterBy('template', 'journal')->filterBy('volume', '!=', '')->groupBy('volume') as $volume => $items): ?>
      <div class="row">
        ...
        <?php foreach ($items as $issue): ?>
          <a href="<?php echo $issue->url() ?>" class="col-xs-6 col-sm-3 mb-2">
            <img src="<?php if ($issue->hasImages()): ?><?php echo $issue->images()->first()->url() ?><?php else: ?>http://placehold.it/160x209<?php endif; ?>" class="img-fluid" style="box-shadow: 0 5px 10px rgba(0,0,0,.25);">
